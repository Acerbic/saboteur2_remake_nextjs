//{% extends '../layout-subpages-mus.twig' %}
const audiofile = 'sab2_endika' 

export default () => (
    <>
        <img src="static/images/endika.jpg" className="bordered left" alt="Фото Эндика Фернандез" width="600" height="399"/>
        <p>
            Saboteur 2 techno remix - REASON 4 - <br/>
            Эндика Мейза Фернандез (Endika Mieza Fernandez) (Испания)<br/>
            Еще называет себя - DJ The BOSS<br/>
            1981 года рождения.<br/>
            Увлекается созданием музыки.<br/>
            В детстве, в возрасте 11 лет проводил много времени за игрой в Saboteur 2 на компьЮтере ZX Spectrum, который 
            был крайне популярен в то время в Испании.<br/>
            Эндика создал данную композицию как это говорится, с нуля.<br/>
            Великолепный ремикс на тему Saboteur 2 - пожалуй лучшая работа по данной теме.<br/>
            Слушаем - наслаждаемся.<br/>
        </p>
    </>
)